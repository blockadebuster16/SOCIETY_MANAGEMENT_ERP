      ...
      <header
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ease-in-out ${
          isTransparent
            ...
            : 'bg-white/95 backdrop-blur-xl border-b border-slate-200/50 shadow-lg shadow-black/5'
        }`}
      >
            ...
            {onToggleSidebar ? (
              <div className="flex items-center gap-3">
                ...
                <Link to="/" className="flex items-center gap-2.5">
                  ...
                  <span className="text-society-primary font-black text-base tracking-wider hidden sm:block">
                    SUYASH PRIDE
                  </span>
                </Link>
              </div>
                ...
                <>
                  ...
                  <div className="relative">
                    ...
                    >
                      ...
                      <ChevronDown className={`w-3 h-3 transition-transform ${profileDropdownOpen ? 'rotate-180' : ''} ${isTransparent ? 'text-white' : 'text-slate-500'}`} />
                    </button>

                    {profileDropdownOpen && (
                      <>
                        <div onClick={() => setProfileDropdownOpen(false)} className="fixed inset-0 z-40" />
                        <div className="absolute right-0 mt-2 w-56 rounded-xl bg-white border border-slate-200 shadow-2xl py-2 z-50 animate-in fade-in slide-in-from-top-2 duration-200">
                          <div className="px-4 py-3 border-b border-slate-100">
                            <p className="text-sm font-bold text-slate-900 truncate">
                              {user?.first_name} {user?.last_name}
                            </p>
                            <p className="text-xs text-slate-400 truncate mt-0.5">{user?.email}</p>
                            <span className="inline-block mt-1.5 text-[9px] font-bold bg-[#D4AF37]/15 text-[#8B6914] px-2 py-0.5 rounded-full uppercase tracking-widest">
                              {user?.role?.replace('_', ' ')}
                            </span>
                          </div>
                          <Link
        ...
        {!onToggleSidebar && mobileMenuOpen && (
          <div className="lg:hidden bg-white/98 backdrop-blur-xl border-t border-slate-200/50 shadow-2xl">
