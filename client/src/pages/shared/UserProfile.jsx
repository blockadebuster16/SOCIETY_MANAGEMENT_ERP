...
function UserProfile() {
  ...
  // Base Tabs
  const tabs = [
    { id: 'registry', label: 'Profile Info', icon: <User className="w-4 h-4" /> }
  ];

  // Add extra tabs for resident
  if (user?.role === 'resident') {
    tabs.push({ id: 'family', label: 'Family Roster', icon: <Users className="w-4 h-4" /> });
    tabs.push({ id: 'vehicles', label: 'Vehicle Records', icon: <Car className="w-4 h-4" /> });
  }
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in duration-300">
      ...
      <div className="border-b border-slate-200 dark:border-slate-800 pb-4">
        <h2 className="text-2xl font-bold text-society-primary dark:text-white">User Profile</h2>
        <p className="text-slate-550 dark:text-slate-400 text-xs mt-1">Review official allocations, records, and personal info.</p>
      </div>

      {/* Tab Selectors */}
      <div className="flex border-b border-slate-200 dark:border-slate-800 pb-1 gap-4 overflow-x-auto no-scrollbar">
        {tabs.map((tab) => (
          <button
            key={tab.id}
            onClick={() => setActiveTab(tab.id)}
            className={`flex items-center gap-2 px-4 py-2 border-b-2 text-xs font-semibold tracking-wide transition whitespace-nowrap ${
              activeTab === tab.id
      ...
      <div className="min-h-[350px]">
        <AnimatePresence mode="wait">
          {activeTab === 'registry' && (
            ...
            >
              <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-6 shadow-sm space-y-4 transition-theme">
                <h3 className="font-bold text-society-primary dark:text-[#D4AF37] text-sm border-b border-slate-100 dark:border-slate-800 pb-2 uppercase tracking-wide">Personal Details</h3>
                <div className="space-y-3.5 text-xs text-slate-800 dark:text-slate-250">
                  <div className="flex justify-between"><span className="text-slate-400">Full Name:</span><span className="font-bold">{user?.first_name} {user?.last_name}</span></div>
                  <div className="flex justify-between"><span className="text-slate-400">Email Address:</span><span className="font-bold">{user?.email}</span></div>
                  <div className="flex justify-between"><span className="text-slate-400">Registered Phone:</span><span className="font-bold">{user?.phone || 'N/A'}</span></div>
                  <div className="flex justify-between"><span className="text-slate-400">System Role:</span><span className="font-bold bg-[#D4AF37]/10 text-society-primary dark:text-[#D4AF37] px-2 py-0.5 rounded capitalize">{user?.role}</span></div>
                  
                  {user?.role === 'resident' && (
                    <div className="flex justify-between"><span className="text-slate-400">Flat allocation:</span><span className="font-bold">{user?.unit_assignment || 'Not Assigned'}</span></div>
                  )}

                  <div className="flex justify-between"><span className="text-slate-400">Account Status:</span>
                    <span className="font-bold text-emerald-600 bg-emerald-50 dark:bg-emerald-900/20 px-2 py-0.5 rounded">{user?.status || 'Active'}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-6 shadow-sm space-y-4 transition-theme flex flex-col justify-between">
                <div>
                  <h3 className="font-bold text-society-primary dark:text-[#D4AF37] text-sm border-b border-slate-100 dark:border-slate-800 pb-2 uppercase tracking-wide">Account Information</h3>
                  <div className="space-y-3 pt-2 text-xs text-slate-550 dark:text-slate-400 leading-normal">
                    <p className="flex items-start gap-2">
                      ...
                      <span>Your role determines your access to modules within the portal. Keep your credentials secure.</span>
                    </p>
                    <p className="flex items-start gap-2">
                      <Info className="w-4 h-4 text-[#D4AF37] flex-shrink-0 mt-0.5" />
                      <span>For updating ownership title, login details, or verifying records, please visit the society office or contact the admin committee.</span>
                    </p>
                  </div>
                </div>
          ...
          {activeTab === 'family' && user?.role === 'resident' && (
            // 2. FAMILY ROSTER (Residents Only)
            <motion.div
              key="family"
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
          ...
          {activeTab === 'vehicles' && user?.role === 'resident' && (
            // 3. VEHICLE RECORDS (Residents Only)
            <motion.div
              key="vehicles"
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
